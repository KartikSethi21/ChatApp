...
const socketAuthenticator=async(err,socket,next)=>{
    try {
        ...
        const autToken= socket.request.cookies[Chat_Token];

        if(!autToken) return next(new ErrorHandler("Please login to access this route",401));

        const decodedData=jwt.verify(autToken,process.env.JWT_SECRET);
